...
const saveUser = async (user) => {
  try{
    ...
    {
        getUser.userName = user?.userName ? user.userName : getUser.userName;
        getUser.passWord = user?.passWord ? bcrypt.hashSync(user.passWord, 10) : getUser.passWord;
        getUser.fullName = user?.fullName ? user.fullName : getUser.fullName;
        getUser.birthDate = user?.birthDate ? new Date(user.birthDate) : getUser.birthDate;
        ...
        getUser.achivement = user?.achivement ? user.achivement : getUser.achivement;
    ...
    else 
    {
      ...
      if(getUser instanceof user && getUser)
        return getUser;
...
const initDataUser = async () => {
    ...
    for(const prop in readUsers)
      await saveUser(readUsers[prop]);
} 
