...
import medias from './medias.mongo.js';
...
const saveMedia = async (media) => {
    try {
        ...
        if (getMedia instanceof medias && getMedia) {
            ...
            getMedia.transcript = media?.transcript
                ? media.transcript
                : getMedia.transcript;
            ...
            if (getMedia instanceof medias && getMedia) return getMedia.id;
...
const initDataMedia = async () => {
    ...
    const json = readFileSync(path.join(__dirname, 'src/data/media.json'));
...
export { initDataMedia, findMaxId, streamUpload, saveMedia };
