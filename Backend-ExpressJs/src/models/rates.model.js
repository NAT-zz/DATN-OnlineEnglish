...
import rates from './rates.mongo.js';
...
const saveRate = async (rate) => {
    try {
        ...
        if (getRate instanceof rates && getRate) {
            ...
            if (getRate instanceof rates && getRate) return getRate;
...
const initDataRate = async () => {
    ...
    const json = readFileSync(path.join(__dirname, 'src/data/rate.json'));
...
export { initDataRate, findMaxId, saveRate };
