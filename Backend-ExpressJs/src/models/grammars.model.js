...
const grammars = require('./grammars.mongo');
const { QUESTION_TYPE } = require('../utils/Constants');
const grammarsMongo = require('./grammars.mongo');
...
const saveSelect = async(grammar) => {
    try {
        ...
        if(getGrammar instanceof grammars && getGrammar)
        {
            ...
            getGrammar.answers = grammar?.answers ? grammar.answers : getGrammar.answers;
            getGrammar.grammarType = grammar?.grammarType ? grammar.grammarType : getGrammar.grammarType;
            getGrammar.questionType = grammar?.questionType ? grammar.questionType : getGrammar.questionType;
            getGrammar.media = grammar?.media ? grammar.media : getGrammar.media;;
            getGrammar.level = grammar?.level ? grammar.level : getGrammar.level;
            getGrammar.taskType = grammar?.taskType ? grammar.taskType : getGrammar.taskType;
...
const initDataGrammar = async() => {
    ...
    const json = readFileSync(
      path.join(__dirname, '../data/grammar.json')
    );
