...
const { GRAMMAR_TYPE, QUESTION_TYPE, SKILLS, COURSE, TASK_TYPE } = require('../../utils/Constants');
const { StatusCodes } = require('http-status-codes');
const topics = require('../../models/topics.mongo');
const grammars = require('../../models/grammars.mongo');
const medias = require('../../models/medias.mongo');
const users = require('../../models/users.mongo');

const { saveTopic } = require('../../models/topics.model');
const { streamUpload } = require('../../models/medias.model');
const { saveMedia } = require('../../models/medias.model');
const { saveGrammar } = require('../../models/grammars.model');
    ...
    try{
        ...
        if(getTopic instanceof topics && getTopic)
        {
            const objBeaty = {
                ...
                preparationTask: getTopic?.preparationTask ? getTopic.preparationTask : null,
                tasks: [],
                medias: [],
                provider: null
            };
            ...
            {
                const getPre = await grammars.findOne({ id: getTopic.preparationTask }, ("-_id -__v -r"));
            ...
            if(getTopic.tasks.length > 0)
            {
                ...
                {
                    let getTask = await grammars.findOne({ id: val }, ("-_id -__v -r"));
            ...
            if(getTopic.media.length > 0)
            {
                ...
                {
                    let getMedia = await medias.findOne({ id: val }, ("-_id -__v -r"));
            ...
            {
                const getUser = await users.findOne({ id: getTopic.idProvider });
...
const createTopic = async(req, res) => {
    try{
        if(!req.body.name || !req.body.preview || !req.body.topicSkill ||
                !req.body.instruction  || !req.body.level)
            return makeSuccessResponse(res, StatusCodes.BAD_REQUEST, {
...
const addTasksToTopic = async(req, res) => {
    try{
        ...
        if(preTaskId)
            getTopic.preparationTask = preTaskId;
        else
            throw new Error(`Cannot save preparation task`);
        ...
        for(const val of tasks)
        {
            ...
            if(!getTopic.tasks.includes(taskId))
                getTopic.tasks.push(taskId);
        }
        await getTopic.save();
        return makeSuccessResponse(res, StatusCodes.OK, {
