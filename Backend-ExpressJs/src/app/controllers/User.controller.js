...
const { redisClient, getValue, setValue } = require('../../services/redis')
const { findMaxId } = require('../../models/users.model');
const { JWT_ACCESS_SECRET, JWT_REFRESH_SECRET, JWT_ACCESS_TIME, JWT_REFRESH_TIME, TOKENS } = require('../../utils/Constants');
...
const registerUser = async (req, res) => {
    try
    {
        ...
        const reqFullname = req.body?.fullname?.trim() ? req.body.fullname : null;
...
const loginUser = async (req, res) => {
    try {
        ...
        else if (!getUser.status)
        {
            ...
            const refreshToken = await generateRefreshToken(getUser.userName, getUser.role);
            ...
            if (getUser && getUser instanceof users)
            {
                ...
                if (newToken instanceof tokens && newToken)
                {
                    ...
                    transporter.sendMail(mailOptions, err => {
                        ...
                        return makeSuccessResponse(res, StatusCodes.OK, {
                            message: 'An email has been sent to ' + getUser.email + '. It will be expired after 1 minute.'
                        })
                    });
                }
                else
...
const editProfile = async (req, res) => {
    ...
    try{
        ...
        const user = await users.findOne({email: req.body.email.trim()}, ('-_id -__v'));
        ...
        {   
            Object.assign(user, {
                ...
                avatar: req?.file ? (await streamUpload(req.file)).url : user.avatar,
            })
            if(req.body.password)
            {
                if(!req.body.currentpassword)
                    throw new Error('Current password needed for changing password');
                else{
                    if(req.body.password !== req.body.confirmpassword)
                        throw new Error(`Password not match`);
                    else
                    {
                        if (bcrypt.compareSync(req.body.currentpassword.trim(), user.passWord))
                        {   
                            Object.assign(user, {
