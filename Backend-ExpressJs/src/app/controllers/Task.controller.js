...
const { findRandomTasks } = require('../../models/grammars.model');
const { GRAMMAR_TYPE, QUESTION_TYPE, SKILLS } = require('../../utils/Constants');
...
const checkGrammarAnswer = async (type, data) => {
    ...
    //     ]
    // }
    const correctResult = [], wrongResult = [];
    for(const val of data)
    {
        ...
        if(getSentence)
        {
            if(getSentence.key === val.answer)
                correctResult.push(val.id);
            else
                wrongResult.push(val.id);
        }
    }
    return {
    ...
    try{
        ...
        switch(skill.toUpperCase())
        {
            case SKILLS.GRAMMAR:
                ...
                if(result)
                {
                    const percent = ((result.correctResult.length / req.body.data.length)*100).toFixed();
                    return makeSuccessResponse(res, StatusCodes.OK, {
...
const deleteGrammar = async(req, res) => {
    try {
        ...
        {
            case SKILLS.GRAMMAR:
                const deletedCount = (await grammars.deleteOne({ id: reqId })).deletedCount;
                if(deletedCount)
                    return makeSuccessResponse(res, StatusCodes.OK, {
...
const getGrammar = async(req, res) => {
    try{
        ...
        {
            case SKILLS.GRAMMAR:
                const findGrammar = await grammars.findOne({ id: reqId }, ("-_id -__v -r"));
